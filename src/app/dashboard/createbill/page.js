...
const CreateBill = () => {
        ...
        const fetchClients = async () => {
            try {
                ...
                if (res.data?.data) {
                    ...
                    const add = [
                        ...new Set(res.data.data.map((c) => c.address)),
                    ];
                    setAddress(add);
                }
            } catch (error) {
    ...
    useEffect(() => {
        if (!clientaddress) setFilteredClients(clients);
            ...
            setFilteredClients(
                clients.filter((c) => c.address === clientaddress)
            );
    ...
    useEffect(() => {
        const updatedItems = items.map((item) => {
            ...
            const newRate =
                taxRate > 0 ? origRate / (1 + taxRate / 100) : origRate;
            return {
                ...item,
